...
从 Hermes Agent 的 `state.db` 数据库导出会话历史为可读 Markdown 文件的通用技能。
...
- "导出会话历史" / "备份对话"
- "保存完整对话到文档"
- "把 GitHub-Know 项目的会话导出来"

## 功能

| 功能 | 说明 |
|------|------|
| 按关键词匹配 | `title LIKE '%github-know%'` 过滤项目相关会话 |
...
```bash
# 导出包含 "github-know" 的会话
python3 scripts/export_sessions.py --keyword "github-know" --output docs/sessions/
